...
export class OptimizelyService {
	...
	getExperiment (flagKey: string): any {
	  if (this.activeInstance != null) {
	    const ret = this.activeInstance.get().experiments.filter(e => e.key == flagKey)
	    if (ret.length > 0) {
	      return ret[0]
	    }
	    var e:any = null
	    this.activeInstance.get().groups.forEach(g => {
	      const ret = g.experiments.filter(e => e.key == flagKey)
	...
	getFeatureFlag (flagKey: string): any {
	  if (this.activeInstance != null) {
			 const ret:[any] = this.activeInstance.get().featureFlags.filter(f => f.key == flagKey)
			 if (ret.length > 0) {
				 return ret[0]
			 }
	  }
	...
	getFlags (): string[] {
	  if (this.activeInstance == null) {
	    ...
	    const conf = this.activeInstance.get()
	    if (conf == null) {
	    ...
	    for (const e of conf.featureFlags) {
	      arr.push(e.key)
	    }
	    return arr
	  }
	...
	getExperiments (): string[] {
	  if (this.activeInstance == null) {
	    ...
	    for (const e of this.activeInstance.get().experiments) {
	      arr.push(e.key)
	    }

	    for (const g of this.activeInstance.get().groups) {
